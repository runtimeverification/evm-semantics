...
tests/%.run-web3: tests/%.in.json
	PORT=`tests/web3/get_port.py`; $(client_kompiled) -p $$PORT & while ! netcat -z 127.0.0.1 $$PORT; do sleep 0.1; done; cat $^ | netcat 127.0.0.1 $$PORT -q 0 | diff - tests/$*.out.json; RESULT=$$?; pkill kevm-client -P $$$$; [ $$? -eq 0 ]
