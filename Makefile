...
.PHONY: all clean clean-submodules distclean \
        deps all-deps llvm-deps haskell-deps repo-deps system-deps k-deps ocaml-deps plugin-deps libsecp256k1 libprocps libff \
...
$(DEFN_DIR)/node/$(MAIN_DEFN_FILE)-kompiled/interpreter: $(node_files) $(DEFN_DIR)/node/$(MAIN_DEFN_FILE)-kompiled/plugin/proto/msg.pb.cc $(libprocps_out)
	@echo "== kompile: $@"
...
$(node_kompiled): $(DEFN_DIR)/node/$(MAIN_DEFN_FILE)-kompiled/interpreter $(libprocps_out)
	mkdir -p $(DEFN_DIR)/vm
...
$(llvm_kompiled): $(llvm_files) $(libprocps_out)
	@echo "== kompile: $@"
	$(K_BIN)/kompile --debug --main-module $(MAIN_MODULE) --backend llvm \
