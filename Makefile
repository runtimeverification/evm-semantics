...
.PHONY: all clean deps all-deps llvm-deps haskell-deps repo-deps system-deps k-deps ocaml-deps plugin-deps libsecp256k1 libff \
        ...
        test-prove test-prove-gen test-parse test-interactive test-interactive-help test-interactive-run test-interactive-prove \
...
smoke_tests_prove=$(TEST_SPECS_DIR)/examples/sum-to-n-spec.k \
                  $(TEST_SPECS_DIR)/ds-token-erc20/transfer-failure-1-a-spec.k
