...
# The main target: $(KEVM_LIB_DIR)/$(KEVM_LIB)
...
krypto-plugin: init-submodules
	$(MAKE) -C $(CRYPTO_PLUGIN_DIR) -j$(shell nproc)

evm-semantics: krypto-plugin libulmkllvm
	kompile \
		...
		-ccopt -lcrypto \
		-ccopt -lsecp256k1 \
		-ccopt $(CRYPTO_PLUGIN_DIR)/build/krypto/lib/krypto.a \
