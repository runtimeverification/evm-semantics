...
module SCHEDULE
 ...
 // -----------------------------------------------------------------
    ...
    syntax ScheduleFlag ::= "Gselfdestructnewaccount" | "Gstaticcalldepth" | "Gemptyisnonexistent" | "Gzerovaluenewaccountgas"
                          ...
                          | "Ghasfloorcost"           | "Ghasclz"          | "Ghaseip7928"
 // ---------------------------------------------------------------------------------------
```
...
```k
 ...
 // ---------------------------------------------------------------
    ...
    syntax ScheduleConst ::= "Gzero"         | "Gbase"         | "Gverylow"      | "Glow"              | "Gmid"               | "Ghigh"            | "Gextcodesize"
                           ...
                           | "Gbls12PairingCheckAdd"           | "Gauthbase"     | "Gbls12mapfp2tog2"  | "Gtxdatafloor"       | "Gbalitemcost"
 // --------------------------------------------------------------------------------------------------------------------------------------------
```
...
```k
 ...
 // -----------------------------------------------------------------------------
    ...
    rule [Gbls12g1addDefault]:           Gbls12g1add           < DEFAULT > => 0
    rule [Gbls12g1mulDefault]:           Gbls12g1mul           < DEFAULT > => 0
    rule [Gbls12g2addDefault]:           Gbls12g2add           < DEFAULT > => 0
    rule [Gbls12g2mulDefault]:           Gbls12g2mul           < DEFAULT > => 0
    rule [Gbls12PairingCheckMulDefault]: Gbls12PairingCheckMul < DEFAULT > => 0
    ...
    rule [GbalitemcostDefault]:          Gbalitemcost          < DEFAULT > => 0
    ...
    rule [Ghaseip7928Default]:             Ghaseip7928             << DEFAULT >> => false
```
...
```k
 ...
 // -----------------------------------------------------------------------------------
    rule [GbalitemcostAmsterdam]:              Gbalitemcost              < AMSTERDAM > => 2000
    ...
    rule [SCHEDCONSTAmsterdam]:                SCHEDCONST                < AMSTERDAM > => SCHEDCONST < OSAKA >
      requires notBool ( SCHEDCONST ==K Gbalitemcost
                  orBool SCHEDCONST ==K Gmaxblobgas
                  orBool SCHEDCONST ==K Gtargetblobgas
                  orBool SCHEDCONST ==K Blobbasefeeupdatefraction
                       )

    rule [Ghaseip7928Amsterdam]: Ghaseip7928 << AMSTERDAM >> => true
    rule [SCHEDFLAGAmsterdam]:   SCHEDFLAG   << AMSTERDAM >> => SCHEDFLAG << OSAKA >>
      requires notBool ( SCHEDFLAG ==K Ghaseip7928 )
```
