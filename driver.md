...
```{.k .standalone}
 ...
 // -------------------------------------------------------------------------------------
    ...
    rule <k> load "exec" : { "code" : (CODE:OpCodes)   } => . ... </k> <programBytes> _ => #asmOpCodes(CODE) </programBytes> <jumpDests> _ => #computeValidJumpDests(#asmOpCodeS(CODE)) </jumpDests> <schedule> SCHED </schedule>
    rule <k> load "exec" : { "code" : (CODE:ByteArray) } => . ... </k> <programBytes> _ => CODE </programBytes> <jumpDests> _ => #computeValidJumpDests(CODE) </jumpDests> <schedule> SCHED </schedule>
```
