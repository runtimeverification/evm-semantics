...
def exec_compile(args: Namespace) -> None:
    res = solc_compile(args.contract_file, profile=args.profile)
    print(json.dumps(res))


def exec_gst_to_kore(args: Namespace) -> None:
...
def exec_solc_to_k(args: Namespace) -> None:
...
def exec_foundry_to_k(args: Namespace) -> None:
    kevm = KEVM(_definition_dir(args), profile=args.profile)
    empty_config = kevm.definition.empty_config(KSort('KevmCell'))
