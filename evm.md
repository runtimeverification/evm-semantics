...
```k
 ...
 // -------------------------------------------
    ...
    rule <k> #mkCreate ACCTFROM ACCTTO VALUE INITCODE
          => #touchAccounts ACCTFROM ACCTTO
          ~> #accessAccounts ACCTFROM ACCTTO
 ...
 // ---------------------------------------------------
    ...
    rule <k> #mkCodeDeposit _ACCT => #popCallStack ~> #popWorldState ~> 0 ~> #push ... </k>
         ...
         <output> OUT => .ByteArray </output>
      requires #sizeByteArray(OUT) >Int maxCodeSize < SCHED >
       orBool notBool #isValidCode(OUT, SCHED)
...
```k
 ...
 // --------------------------------------------------------------------
    ...
    syntax ScheduleConst ::= "Gzero"            | "Gbase"              | "Gverylow"      | "Glow"          | "Gmid"        | "Ghigh"
                           ...
                           | "Gwarmstorageread" | "Gaccesslistaddress" | "Gaccessliststoragekey" | "reservedStartingByte"
 // ---------------------------------------------------------------------------------------------------------------------
```
...
```k
 ...
 // ----------------------------------------------------------------------------------
    ...
    rule SCHEDCONST           < LONDON > => SCHEDCONST < BERLIN >
      requires notBool ( SCHEDCONST ==K reservedStartingByte )
