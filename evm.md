...
```{.k .uiuck .rvk}
 ...
 // -------------------------------
    ...
    rule <op> #calcGas(NEEDEDMEM, PREVMEM, GEXEC) =>
            ( maxInt(PREVMEM, NEEDEDMEM) -Int PREVMEM ) *Int Gmemory +Int GEXEC
    ... </op>

```

```{.k .uiuck .rvk}
    syntax InternalOp ::= #gasExec ( OpCode ) | #calcGas ( Int ) | #calcGas( Int, Int ) | #calcGas (Int, Int, Int)
