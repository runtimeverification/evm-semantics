      ...
      stages {
        ...
        stage('Test') {
          ...
          parallel {
            ...
            stage('Proofs (Java)')      { steps { lock("kevm-java-${env.NODE_NAME}")    { sh 'make test-prove       -j3 TEST_SYMBOLIC_BACKEND=java'    } } }
            stage('Proofs (Haskell)')   { steps { lock("kevm-haskell-${env.NODE_NAME}") { sh 'make test-prove       -j6 TEST_SYMBOLIC_BACKEND=haskell' } } }
          }
        }
